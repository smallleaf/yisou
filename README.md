...
1. GenerateStart 脚本启动程序，ant打包项目的时候，会启动该程序，生成启动关闭项目的脚本，这样可以方便项目启动关闭，特别是jetty内嵌式。
2. YiSouClient 客户端，网站的主要项目模块。主要做网页显示。
3. YiSouService 调用google接口，将它单独写成一个模块，以后可以一直不动它。
4. YiUtils 自己封装的一些工具类，比如HttpClient的封装，还有响应模板的封装等等。将常用的打包成jar，供其他项目使用。
